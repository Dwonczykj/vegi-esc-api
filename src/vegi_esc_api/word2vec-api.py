...
from vegi_esc_api.logger import set_log_level, LogLevel
from vegi_esc_api.protocols import ESCRatingExplainedResult
...
def initApp(app, args:argparse.Namespace|None=None):
    ...
    if args:
        ...
        model_path = args.model if args.model else "./models/GoogleNews-vectors-negative300.bin" #"./model.bin.gz"
        ...
        host = "localhost"
        path = "/word2vec"
        port = int(os.environ.get('PORT', 5002)) # allow heroku to set the path, no need for python-dotenv package to get this env variable either...
...
if __name__ == '__main__':
    ...
    logger.info(f'Thread name running app is "{__name__}"') # Thread name running app is "word2vec-api" if command run is `gunicorn --bind 127.0.0.1:5002 word2vec-api:gunicorn_app --timeout 90`
    runApp = initApp(app=app)
    gunicorn_app = app # gunicorn --bind 127.0.0.1:5002 word2vec-api:gunicorn_app --timeout 90 --log-level=debug
